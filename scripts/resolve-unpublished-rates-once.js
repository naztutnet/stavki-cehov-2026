...
if(targets.some(r=>r.status!=='check'))throw new Error('Expected all six target rows to still have status check');
...
if(!updates.includes('Публичные ставки вместо «требует проверки»')){
  updates=updates.replace('const UPDATES = [', `const UPDATES = [\n  {date:'13.08.2026',title:'Публичные ставки вместо «требует проверки»',text:'Для 4 каскадёрских и 2 sound post позиций зафиксирован результат проверки: профессии подтверждены, но публичные тарифы не опубликованы. Неподтверждённые суммы убраны; стоимость указана как «по договорённости», рыночные ориентиры показываются отдельно.'},`);
  fs.writeFileSync('updates.js',updates);
}
console.log('Resolved six unpublished-rate rows:', targets.map(r=>`${r.id}:${r.prof}`).join(' | '));
