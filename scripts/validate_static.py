...
def validate_css_braces(content, name):
    ...
    while i<len(content):
        ...
        if quote:
            ...
            if ch in "'\"": quote=ch
                ...
                depth-=1
                if depth<0: fail(f'Extra closing brace in {name}')
        i+=1
    if comment or quote or depth: fail(f'Unclosed construct in {name}')
...
mw=re.search(r'<meta property="og:image:width" content="(\d+)">',index)
mh=re.search(r'<meta property="og:image:height" content="(\d+)">',index)
...
for required in (
    ...
    if required not in index: fail(f'Missing current social preview metadata: {required}')

assets={
    'app.css':r'<link rel="stylesheet" href="app\.css\?v=([^"]+)">',
    'production-type-filter.js':r'<script src="production-type-filter\.js\?v=([^"]+)"></script>',
...
if '<lastmod>2026-08-24</lastmod>' not in (root/'sitemap.xml').read_text(encoding='utf-8'): fail('sitemap lastmod was not refreshed')
...
if '"Golos Text"' not in seo_css: fail('seo-pages.css does not use Golos Text')
...
for path in seo_pages:
    ...
    if f'<meta property="og:image" content="{og_url}">' not in page: fail(f'{path} does not use the current social preview image')
    if '<meta name="robots" content="index, follow, max-image-preview:large">' not in page: fail(f'{path} is missing robots metadata')
    if '/seo-pages.css?v=' not in page: fail(f'{path} is missing versioned SEO stylesheet')
    if 'class="seo-shell"' not in page or 'class="seo-sidebar"' not in page: fail(f'{path} is not using the current KinoRates landing-page shell')
...
print(f'Static validation OK: {len(dom_refs)} DOM refs, OG {w}x{h}, current landing-page shell verified')
