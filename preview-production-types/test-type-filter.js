...
assert(Array.isArray(siteUpdates) && siteUpdates.length === 15, "Curated review draft must contain exactly 15 updates");
...
assert(JSON.stringify(counts) === JSON.stringify({ all: 449, "cinema-series": 205, "commercial-media": 156 }), `Unexpected counts: ${JSON.stringify(counts)}`);
