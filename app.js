...
function topbar(route) {
  ...
  return `<header class="topbar"><button class="menu" id="menu">☰</button><div class="breadcrumb">KinoRates <span>/</span> ${label}</div><div class="top-actions"><span>● Данные актуальны</span><button class="quiet" data-feedback data-feedback-topic="Хочу поделиться актуальной ставкой">Поделиться ставкой</button></div></header>`;
}
...
function openModal(type, feedbackTopic) { $("#modalRoot").innerHTML = modal(type, feedbackTopic); document.querySelectorAll("[data-close]").forEach((b) => (b.onclick = () => ($("#modalRoot").innerHTML = ""))); if (type === "feedback") bindFeedbackForm(); }
