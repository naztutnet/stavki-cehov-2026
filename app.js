...
const ICG_CALCULATOR_URL = "https://icguild.org/calculator";
...
const sourceYear = (source) => Number(String(source?.date || source?.name || "").match(/(?:19|20)\d{2}/)?.[0] || 0);
const latestSourceYear = Math.max(0, ...S.map(sourceYear));
const sourceDateTone = (source) => {
  const year = sourceYear(source);
  if (/рыночный ориентир|калькулятор/i.test(String(source?.name || ""))) return "market";
  if (year === latestSourceYear) return "current";
  if (year >= latestSourceYear - 3) return "previous";
  return "archive";
};
...
const sections = [
  ["home", "≡", "Ставки"], ["knowledge", "▤", "Как читать"],
  ["resources", "↗", "Документы"], ["market", "⌁", "Рынок"], ["updates", "◷", "Обновления"],
];
...
function topbar(route) {
  const label = sections.find((x) => x[0] === route)?.[2] || ({ about: "О проекте", contacts: "Контакты", projects: "Смета", article: "Материал" })[route] || "KinoRates";
...
const productionTypeDescriptions = {
  all: "Все ставки из базы.",
  "cinema-series": "Ставки для кино и сериалов.",
  "commercial-media": "Только ставки, где прямо указаны реклама, клип, музыкальное видео, ТВ или шоу. Ставки из кино сюда не переносим.",
};
...
function home() {
  ...
  return `<div class="registry-page"><section class="registry-intro"><div><span class="eyebrow">СТАВКИ · ОБНОВЛЕНО 20.08.2026</span><h1>Ставки</h1><p>Выберите формат, найдите профессию и проверьте условия. Это ориентиры — финальную сумму стороны согласуют сами.</p></div><button class="feedback-link" data-feedback>Есть ошибка? Сообщите</button></section><div class="source-strip"><div class="registry-stats"><b>${R.length}</b><span>позиций</span><b>${current}</b><span>сверено в 2026</span></div><a href="https://docs.google.com/spreadsheets/d/19GyzCN-CKlAehf2u7-hJyL4lzIsNtIu06t0vjsYflt4/edit?gid=0#gid=0" target="_blank" rel="noopener"><span>Первоисточник</span><b>МПК</b><small>открыть таблицу ↗</small></a><a href="https://docs.google.com/spreadsheets/d/1BCgusuck7uhHvDZ2d-nUVyjZHlzrf05286fwpahlwdE/edit?gid=0#gid=0" target="_blank" rel="noopener"><span>Первоисточник</span><b>Точно продюсер</b><small>открыть таблицу ↗</small></a><a href="#resources"><span>Первоисточники</span><b>Документы</b><small>${S.length} документов →</small></a><a class="source-materials-link" href="/stavki-kinotsekhov/"><span>Гайды</span><b>Материалы</b><small>цеха и смета →</small></a></div>${productionTypePanel()}<div class="registry-workspace"><aside class="dept-list"><header><h2>Цеха</h2><span>${depts.length}</span></header><button class="${selectedDept ? "" : "active"}" data-dept="">Все цеха <em>${typedRates.length}</em></button>${depts.map((d) => `<button class="${selectedDept === d ? "active" : ""}" data-dept="${esc(d)}">${esc(d)} <em>${typedRates.filter((x) => x.dept === d).length}</em></button>`).join("")}</aside><section class="registry-main"><div class="registry-controls"><div class="ai-search main-search"><span>⌕</span><input id="rateSearch" value="${esc(rateQuery)}" placeholder="Профессия, цех или условие"></div><select id="rateStatus" aria-label="Фильтр по статусу"><option value=""${selectedRateStatus ? "" : " selected"}>Все статусы</option><option value="new"${selectedRateStatus === "new" ? " selected" : ""}>Сверено в 2026</option><option value="old"${selectedRateStatus === "old" ? " selected" : ""}>Архив и ориентиры</option></select><button class="primary budget-open" data-new data-budget-indicator>Смета · ${budgetItems.length}</button></div><div class="registry-layout"><div><div class="suggested"><span>Быстрый поиск:</span><button>оператор-постановщик</button><button>гаффер</button><button>второй режиссёр</button><button>монтаж</button></div><div id="rateTable"></div></div></div></section></div><div class="toast" id="toast" role="status" aria-live="polite"></div></div>`;
}
function homeV2() {
  const current = R.filter((x) => ["fresh2026", "official2026"].includes(x.status)).length, historical = R.length - current;
  return home().replace(`<div class="registry-stats"><b>${R.length}</b><span>позиций</span><b>${current}</b><span>сверено в 2026</span></div>`, `<div class="registry-stats"><div><span>База</span><b>${R.length}</b><small>всего позиций</small></div><div><span>2026</span><b>${current}</b><small>сверено</small></div><div><span>Архив</span><b>${historical}</b><small>и рынок</small></div></div>`);
}
function projects() {
...
function projectsV2() {
  ...
  return pageHead("ЧЕРНОВОЙ РАСЧЁТ", "Рабочая смета", "Добавьте ставки, задайте количество и период — итог посчитается автоматически. Черновик хранится только в этом браузере.", actions) + `<div class="budget-workspace"><section class="budget-sheet">${content}</section><aside class="budget-total"><span>Позиций</span><b>${budgetItems.length}</b><div><span>Без налога</span><strong>${rub(net)}</strong></div><div><span>С налогом</span><strong>${rub(gross)}</strong></div><p>Налог считаем делением на (1 − ставка налога), как в исходной таблице.</p></aside></div>`;
}
function rates() {
...
function knowledge() {
  const legend = [
    ["current", "Письмо 2026", "Свежий документ со ставкой и условиями работы."],
    ["current", "Рекомендации 2026", "Свежая публикация профессионального объединения."],
    ["previous", "Письмо 2025 / 2024 / 2023", "Проверенный документ прошлых лет. Перед расчётом уточните, действует ли ставка сейчас."],
    ["market", "Рыночный ориентир", "Пример суммы из исследования рынка. Это не обязательная ставка."],
    ["archive", "Архив", "Старый документ. Используйте его только как ориентир и согласуйте сумму отдельно."],
    ["unpublished", "Без публичного тарифа", "Профессия подтверждена, но открытой ставки нет. Сумму нужно согласовать."],
  ];
  return pageHead("КОРОТКАЯ ИНСТРУКЦИЯ", "Как читать ставки", "Сначала выберите формат. Потом проверьте, за что платят, год и первоисточник.") + `<section class="rate-reading-guide"><header><span>ПЕРЕД РАСЧЁТОМ</span><h2>Три шага</h2></header><div class="rate-reading-steps"><article><i>01</i><h3>Выберите формат</h3><p>Ставки для кино и рекламы не смешиваются. Если выбрали рекламу, суммы из кино туда не попадут и не будут пересчитаны.</p><nav><a href="?type=cinema-series#home">Кино и сериал →</a><a href="?type=commercial-media#home">Реклама / клип / ТВ →</a></nav></article><article><i>02</i><h3>Посмотрите, за что платят</h3><p>Смена, день, месяц и проект — разные единицы. Рядом проверьте регион и условия работы.</p><span class="rate-reading-detail">Метки — ниже ↓</span></article><article><i>03</i><h3>Откройте источник</h3><p>Свежий документ надёжнее старого ориентира. Проверьте год и убедитесь, что условия подходят вашему проекту.</p><nav><a href="#resources">Открыть документы →</a></nav></article></div></section><section class="legend-list" id="knowledge-statuses"><header><div><span>ЦВЕТНЫЕ МЕТКИ</span><h2>Что значат цвета</h2></div><p>Цвет относится к конкретной ставке, а не ко всей профессии.</p></header>${legend.map(([kind, title, text]) => `<article><span class="legend-mark ${kind}"></span><div><h3>${title}</h3><p>${text}</p></div></article>`).join("")}</section><aside class="legend-note"><div><b>Перед сметой</b><p>Ещё раз проверьте сумму, единицу расчёта, условия и год документа.</p></div><a class="primary button-link" href="#home">Открыть ставки →</a></aside><aside class="legend-note legend-contribution-note"><div><b>Не нашли ставку или профессию?</b><p>Пришлите название, сумму или документ. Перед публикацией мы всё проверим.</p></div><div class="rate-reading-actions"><button class="primary" data-contribution data-contribution-kind="rate">Предложить ставку</button><button class="quiet" data-contribution data-contribution-kind="profession">Предложить профессию</button></div></aside>`;
}
function resources() {
  const legend = [["current", "Письмо 2026"], ["previous", "Письмо прошлых лет"], ["market", "Рыночный ориентир"], ["archive", "Архив"]];
  return pageHead("ПЕРВОИСТОЧНИКИ", "Письма и документы", "Здесь лежат документы, из которых взяты ставки. Цвет показывает, насколько свежий источник.") + `<div class="resource-status-legend" aria-label="Обозначения дат">${legend.map(([tone, label]) => `<span><b class="status ${tone}">${esc(label)}</b></span>`).join("")}</div><div class="resource-list">${S.map((s, i) => `<a href="${esc(s.url)}" target="_blank" rel="noopener"><i>${String(i + 1).padStart(2, "0")}</i><div><b>${esc(s.name)}</b><span class="status resource-date ${sourceDateTone(s)}">${esc(s.date)}</span></div><em>Открыть ↗</em></a>`).join("")}</div>`;
}
function market() {
  return pageHead("КОНТЕКСТ", "Рынок", "Исследования, опросы и рыночные срезы. Они помогают свериться, но не заменяют ставку и договорённость.") + `<div class="market-grid">${M.map((item) => `<article><div class="market-card-meta"><span>${esc(item.kind)}</span><b>${esc(item.year)}</b></div><h2>${esc(item.title)}</h2><p>${esc(item.text)}</p><small>${esc(item.period)}</small><a href="${esc(item.url)}" target="_blank" rel="noopener">${esc(item.source)} ↗</a></article>`).join("")}</div>`;
}
function siteUpdates() {
  ...
  return pageHead("ЧТО НОВОГО", "Обновления", "Что мы добавили, исправили и перепроверили.") + `<section class="site-update-feed" aria-label="Лента обновлений">${entries}</section>`;
}
function contacts() {
  ...
  return pageHead("KINORATES", "Контакты", "По партнёрствам и совместным проектам.") + `<section class="contact-card"><div class="contact-copy"><span>СОТРУДНИЧЕСТВО</span><h2>Хотите сделать что-то вместе? Напишите.</h2><p>Обсудим проект, партнёрство или развитие KinoRates.</p></div><div class="contact-methods"><button type="button" class="contact-method" data-copy-email aria-label="Скопировать адрес ${CONTACT_EMAIL}">${mailIcon}<span><small>Email</small><b>${CONTACT_EMAIL}</b></span><i data-copy-label aria-live="polite">Скопировать</i></button></div></section>`;
}
function about() {
  return pageHead("KINORATES", "О проекте", "Мы собираем ставки, документы и инструменты для чернового расчёта.", '<div class="page-head-actions"><a class="primary button-link" href="#contacts">Контакты</a><button class="quiet" data-feedback data-feedback-topic="Хочу поделиться актуальной ставкой">Поделиться ставкой</button></div>') + `<div class="about-grid"><section><h2>Ставки и источники<br>в одном месте.</h2><p>Здесь можно найти ставку, увидеть её условия и открыть первоисточник.</p><p>Нужные позиции можно сразу добавить в рабочую смету. Это черновой расчёт, а не готовый бюджет или обязательный тариф.</p></section><aside><div><span>Позиций</span><b>${R.length}</b></div><div><span>Источников</span><b>${S.length}</b></div><div><span>Цеха</span><b>${new Set(R.map((x) => x.dept)).size}</b></div><div><span>Ревизия</span><b>24.08.2026</b></div></aside></div>`;
}
