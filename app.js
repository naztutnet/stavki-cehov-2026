...
function knowledge() {
  ...
  return pageHead("Подсказка по справочнику", "Как читать ставки", "Выберите тип проекта, откройте ставку и проверьте, за что указана сумма, какого года документ и какие в нём условия.") + `<section class="rate-reading-guide"><header><span>ПЕРЕД РАСЧЁТОМ</span><h2>Три простых шага</h2></header><div class="rate-reading-steps"><article><i>01</i><h3>Выберите тип проекта</h3><p>Ставки для кино и сериалов собраны отдельно от рекламы, клипов и ТВ. Сначала откройте нужный раздел. Киношные значения в этот раздел не копируются и не пересчитываются.</p><nav><a href="?type=cinema-series#home">Кино и сериал →</a><a href="?type=commercial-media#home">Реклама / клип / ТВ →</a></nav></article><article><i>02</i><h3>Проверьте расчёт и условия</h3><p>Посмотрите, за что указана сумма: смену, день, месяц или весь проект. Затем проверьте регион и условия работы.</p><span class="rate-reading-detail">Обозначения — ниже ↓</span></article><article><i>03</i><h3>Проверьте год и источник</h3><p>Свежий документ надёжнее старого ориентира. Откройте первоисточник и убедитесь, что ставка подходит вашему проекту.</p><nav><a href="#resources">Цеховые письма →</a></nav></article></div></section><section class="legend-intro" id="knowledge-statuses"><div><span>ЦВЕТНЫЕ МЕТКИ</span><h2>Цвет показывает свежесть данных</h2><p>Метка рядом со ставкой подсказывает, откуда взялась сумма и насколько свежий документ.</p></div><ol><li><b>Год</b><span>Когда вышел документ</span></li><li><b>Расчёт</b><span>Смена, месяц, день или проект</span></li><li><b>Условия</b><span>Для какой работы действует сумма</span></li><li><b>Источник</b><span>Где опубликована ставка</span></li></ol></section><section class="legend-list"><header><div><span>ЧТО ЗНАЧАТ МЕТКИ</span><h2>Коротко о каждом цвете</h2></div><p>Метка относится к одной конкретной ставке, а не ко всей профессии.</p></header>${legend.map(([kind, title, text]) => `<article><span class="legend-mark ${kind}"></span><div><h3>${title}</h3><p>${text}</p></div></article>`).join("")}</section><aside class="legend-note"><div><b>Перед переносом в смету</b><p>Откройте ставку и ещё раз проверьте профессию, сумму, единицу расчёта, условия и год документа.</p></div><a class="primary button-link" href="#home">Открыть справочник ставок →</a></aside><aside class="legend-note legend-contribution-note"><div><b>Не нашли ставку или профессию?</b><p>Пришлите сумму, документ или просто название профессии. Перед публикацией мы всё проверим.</p></div><div class="rate-reading-actions"><button class="primary" data-contribution data-contribution-kind="rate">Предложить ставку</button><button class="quiet" data-contribution data-contribution-kind="profession">Предложить профессию</button></div></aside>`;
}
function resources() {
  const legend = [["current", "Письмо 2026"], ["previous", "Письмо прошлых лет"], ["market", "Рыночный ориентир"], ["archive", "Архив"]];
  return pageHead("Документы со ставками", "Цеховые письма", "Письма и рекомендации, из которых взяты ставки. Цвета означают то же самое, что и в справочнике ставок.") + `<div class="resource-status-legend" aria-label="Обозначения дат">${legend.map(([tone, label]) => `<span><b class="status ${tone}">${esc(label)}</b></span>`).join("")}</div><div class="resource-list">${S.map((s, i) => `<a href="${esc(s.url)}" target="_blank" rel="noopener"><i>${String(i + 1).padStart(2, "0")}</i><div><b>${esc(s.name)}</b><span class="status resource-date ${sourceDateTone(s)}">${esc(s.date)}</span></div><em>Открыть ↗</em></a>`).join("")}</div>`;
}
function market() {
